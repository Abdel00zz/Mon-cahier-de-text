import type { AppConfig, LessonsData } from '../types.js';
 ...
 * repart de 1 à chaque chapitre (ou court sur tout le cahier, au choix).
 *
...
/** Portée du compteur, telle qu'elle est stockée dans la configuration. */
export type ContentNumbering = NonNullable<AppConfig['contentNumbering']>;

export const DEFAULT_CONTENT_NUMBERING: ContentNumbering = { enabled: true, scope: 'chapter' };
 ...
 */
export const buildContentNumbers = (lessons: LessonsData, numbering: ContentNumbering = DEFAULT_CONTENT_NUMBERING): Map<string, string> => {
    const numbers = new Map<string, string>();
    if (!numbering.enabled) return numbers;
    const counters = new Map<string, number>();
    let chapterIndex = -1;
    for (const row of buildLessonRows(lessons)) {
        if (!isNumberableRow(row)) continue;
        if (numbering.scope === 'chapter' && row.indices.chapterIndex !== chapterIndex) {
