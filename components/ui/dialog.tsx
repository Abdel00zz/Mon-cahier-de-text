...
>(({ className, children, hideClose = false, ...props }, ref) => {
  ...
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ...
      dir={isRtl ? "rtl" : "ltr"}
      className={cn(
        "rtl-flow dialog-content fixed inset-x-0 bottom-0 top-auto z-50 grid h-fit min-h-0 max-h-[calc(100dvh-0.5rem)] grid-rows-[auto_minmax(0,1fr)_auto] gap-0 overflow-hidden overscroll-contain rounded-t-[1.25rem] rounded-b-none border border-zinc-200/90 bg-white p-0 text-zinc-900 shadow-[0_-18px_55px_rgba(15,23,42,0.18)] outline-none will-change-transform data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:slide-out-to-bottom-full data-[state=open]:slide-in-from-bottom-full sm:inset-0 sm:m-auto sm:max-h-[calc(100dvh-2.5rem)] sm:w-[calc(100vw-2rem)] sm:rounded-[1.25rem] sm:border-zinc-200/80 sm:shadow-[0_28px_90px_rgba(15,23,42,0.18),0_4px_18px_rgba(15,23,42,0.08)] sm:data-[state=closed]:slide-out-to-bottom-0 sm:data-[state=open]:slide-in-from-bottom-0 sm:data-[state=closed]:zoom-out-95 sm:data-[state=open]:zoom-in-95",
        className
      )}
      {...props}
    >
