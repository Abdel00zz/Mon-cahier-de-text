...
export const CommandPalette: React.FC<CommandPaletteProps> = ({
  ...
  return (
    ...
    >
      <div className="w-full max-w-xl overflow-hidden rounded-2xl border border-slate-700/70 bg-slate-900 text-slate-100 shadow-2xl ring-1 ring-white/10">
        {/* Search header */}
        <div className="flex items-center gap-3 border-b border-slate-800 bg-slate-950/70 px-4 py-3.5">
          ...
          />
          <kbd className="hidden sm:inline-flex items-center gap-1 rounded border border-slate-700 bg-slate-800/80 px-2 py-0.5 text-[10px] font-mono font-medium text-slate-400">
            ESC
          </kbd>
        </div>
        ...
        <div
          ref={listRef}
          className="max-h-[380px] overflow-y-auto p-2 scrollbar-thin scrollbar-thumb-slate-700"
        >
          {filtered.length === 0 ? (
            ...
            filtered.map((item, idx) => {
              ...
              return (
                <button
                  ...
                  className={`flex w-full items-center justify-between gap-3 rounded-xl px-3.5 py-2.5 text-left transition-all cursor-pointer ${
                    isSelected
                ...
                >
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    {item.badge && (
                      <span className="rounded-md border border-indigo-500/30 bg-indigo-500/10 px-2 py-0.5 font-mono text-[10px] font-semibold text-indigo-300">
                        {item.badge}
                      </span>
                    )}
                    {isSelected && (
        ...
        {/* Footer info bar */}
        <div className="flex items-center justify-between border-t border-slate-800 bg-slate-950/60 px-4 py-2.5 text-[11px] text-slate-400 font-mono">
          <div className="flex items-center gap-2">
            <CommandIcon className="h-3.5 w-3.5 text-indigo-400" />
            <span>Fly.io Command Palette</span>
          </div>
