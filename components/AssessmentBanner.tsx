        ...
        <AnimatePresence>
            <motion.div
                ...
                className={`mx-auto mb-4 max-w-5xl rounded-2xl border p-4 shadow-sm ${
                    isUrgent ? 'border-orange-200 bg-orange-50' : 'border-[#B8935A]/30 bg-[#FBF6EE]'
                }`}
                role="status"
            >
                <div className="flex items-start gap-3">
                    <span className={`mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-white shadow-sm ${isUrgent ? 'text-orange-500' : 'text-[#8a6b3d]'}`}>
                    ...
                    <div className="min-w-0 flex-1">
                        ...
                        <ul className="mt-1 space-y-0.5">
                            {upcoming.slice(0, 3).map(a => (
                                <li key={`${a.classId}-${a.id}`} className="text-xs text-slate-600">
                                    <span className="font-semibold text-slate-700">{a.className}</span> — {a.label.split(' — ')[0]}
                                    {' '}<span className={isUrgent && a.inDays <= 3 ? 'font-bold text-orange-600' : 'text-slate-500'}>{delayLabel(a.inDays)}</span>
                                    {a.fenetre && <span className="text-slate-400"> · officiellement {a.fenetre}</span>}
                                </li>
                            ))}
                            {upcoming.length > 3 && (
                                <li className="text-[11px] text-slate-400">+{upcoming.length - 3} autre(s)…</li>
                            )}
                        </ul>
