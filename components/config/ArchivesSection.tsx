...
import { Download, Trash2, CalendarCheck } from '../ui/icons';
...
export const ArchivesSection: React.FC = () => {
    ...
    const handleDelete = (meta: ArchiveMeta) => {
        if (!window.confirm(`Supprimer définitivement l'archive « ${meta.yearLabel} » de cet appareil ?`)) return;
    ...
    return (
        <div className="mt-4 rounded-2xl border border-[#E4D3AC]/80 bg-[#FCF6EA]/40 p-4">
            <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="min-w-0">
                    <h4 className="mb-1.5 text-sm font-bold text-[#2B241D] font-display">Archives des années scolaires</h4>
                    <p className="text-[11px] font-medium leading-relaxed text-[#69604F]">
                ...
                <Button
                    ...
                    className="h-10 shrink-0 rounded-full border-[#E4D3AC] text-xs text-[#B8935A] transition-all hover:bg-[#B8935A] hover:text-white"
                >
            ...
            {archives.length > 0 && (
                <ul className="mt-3 space-y-1.5">
                    {archives.map(meta => (
                        <li
                            key={meta.id}
                            className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-[#E4D3AC]/40 bg-white/70 px-3 py-2"
                        >
                            <div className="min-w-0">
                                <span className="text-xs font-bold text-[#2B241D] font-display">Année {meta.yearLabel}</span>
                                <span className="ml-2 text-[10px] font-semibold text-[#A79C87] font-mono">
                            ...
                            <div className="flex shrink-0 items-center gap-1">
                                <button
                                    ...
                                    className="flex h-8 w-8 items-center justify-center rounded-full text-[#69604F] transition-colors hover:bg-[#FCF6EA] hover:text-[#B8935A]"
                                ...
                                <button
                                    ...
                                    className="flex h-8 w-8 items-center justify-center rounded-full text-[#69604F] transition-colors hover:bg-rose-50 hover:text-rose-600"
