...
import { Check, Plus, Trash2, BookOpen, CalendarRange, GraduationCap, School, FlaskConical } from '../ui/icons';
            ...
            <div dir={isAr ? 'rtl' : 'ltr'} className={`space-y-4 py-0.5 ${isAr ? 'font-ar' : ''}`}>
                    ...
                    {t.steps.map((label, index) => {
                        ...
                        return (
                            <button
                                ...
                                className={`flex min-w-0 flex-1 items-center justify-center gap-1.5 rounded-full border px-2 py-2 text-[11px] font-bold transition-all ${
                                    isCurrent
                    ...
                    <div className="space-y-4" dir="ltr">
                        ...
                        {composedNames.some(Boolean) && (
                            <p className="rounded-xl border border-dashed border-border bg-secondary/50 px-3 py-2 text-xs">
                                ...
                                <span className="font-bold text-foreground/80">{composedNames.filter(Boolean).join(' · ')}</span>
                ...
                <div className="flex items-center justify-between gap-2 pt-1">
                    ...
                    <div className="flex items-center gap-2">
                        {step > 0 && (
                            <Button type="button" variant="secondary" className="h-10 rounded-full px-5 text-xs font-bold" onClick={() => setStep(step - 1)}>
                                {t.back}
                            </Button>
                        )}
                        {step === 0 && (
                            <Button type="button" className="h-10 rounded-full px-6 text-xs font-bold" onClick={() => setStep(1)} disabled={!hasProfile}>
                                {t.next}
                            </Button>
                        )}
                        {step === 1 && (
                            <Button type="button" className="h-10 rounded-full px-6 text-xs font-bold" onClick={createBatch} disabled={!rows.some(r => r.level)}>
                        ...
                        {step === 2 && (
                            <Button
                                type="button"
                                className="h-10 rounded-full px-6 text-xs font-bold"
                                disabled={!hasClasses}
                                onClick={() => {
                                    onClose();
