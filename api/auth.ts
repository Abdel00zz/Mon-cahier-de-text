...
import { normalizePhone, assertName, assertPassword } from './_lib/validate';
import {
  SESSION_COOKIE,
  SESSION_MAX_AGE,
  clearCookie,
...
interface StoredUser {
  ...
  subjects?: string[];
  lastSyncAt?: string;
  /** compte bloqué par l'administrateur : connexion refusée */
  blocked?: boolean;
}
...
const publicUser = (user: StoredUser) => ({ phone: user.phone, nom: user.nom, prenom: user.prenom, cycles: user.cycles ?? [], subjects: user.subjects ?? [] });
...
const handleRegister = async (body: AuthBody, res: ApiResponse) => {
  ...
  await redis.hset(KEYS.adminSnapshots, {
    [phone]: { phone, nom, prenom, cycles, subjects, lastSyncAt: null, classes: [] },
  });
...
const handleMe = async (req: ApiRequest, res: ApiResponse) => {
  ...
  if (user.blocked) {
    // compte bloqué après émission de la session : on la révoque au démarrage
    clearCookie(res, SESSION_COOKIE);
