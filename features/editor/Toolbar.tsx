        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button
              variant="ghost"
              size="icon"
              className={`relative h-9 w-9 cursor-pointer rounded-lg border shadow-sm transition-all ${
                actionRequiredCount > 0
                  ? 'border-amber-300 bg-amber-50 text-amber-700 shadow-amber-100 hover:bg-amber-100'
                  : 'border-border/90 bg-card text-muted-foreground hover:bg-secondary hover:text-primary'
              }`}
          ...
          <DropdownMenuContent align="end" className="w-56 z-[70] rounded-md border border-slate-200 shadow-lg">
            ...
            {(actionRequiredCount > 0 || ignoredActionCount > 0) && onOpenActionCenter && (
              <>
                <DropdownMenuItem onClick={onOpenActionCenter} className="flex cursor-pointer items-center gap-2.5 bg-amber-50/70 py-2.5 text-xs text-amber-800 focus:bg-amber-100 focus:text-amber-900">
                  <CircleAlert className="h-3.5 w-3.5 text-amber-600" />
                  <span className="min-w-0 flex-1 font-bold">Centre d’actions</span>
                  {actionRequiredCount > 0 ? (
                    <span className="rounded-full bg-amber-500 px-2 py-0.5 text-[9px] font-black text-white">{actionRequiredCount}</span>
                  ) : (
                    <span className="text-[9px] font-bold text-slate-400">{ignoredActionCount} ignoré{ignoredActionCount > 1 ? 's' : ''}</span>
                  )}
                </DropdownMenuItem>
                <DropdownMenuSeparator />
              </>
            ...
            <DropdownMenuItem onClick={onOpenEvaluations} className="flex min-h-11 cursor-pointer items-center gap-2.5 rounded-md bg-primary/5 px-2.5 py-2 text-xs text-primary focus:bg-primary/10 focus:text-primary">
              <CalendarCheck className="h-4 w-4" />
              <span className="font-extrabold">Évaluations de cette classe</span>
            </DropdownMenuItem>
            <DropdownMenuSeparator />
