...
export function ClassCardTitle({ name, compact = false, intro }: { name: string; compact?: boolean; intro?: string }) {
    ...
    if (!match) return <span dir="auto" className={`font-sans ${compact ? 'block truncate text-start' : 'line-clamp-2 break-words text-balance text-start'}`}>{renderLabel(name)}</span>;

    return (
        <span dir={titleDirection(match[1])} className={`font-sans ${compact ? 'block truncate text-start' : 'line-clamp-2 break-words text-balance text-start'}`}>
