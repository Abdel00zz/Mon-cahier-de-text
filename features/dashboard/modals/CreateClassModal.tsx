...
export const CreateClassModal: React.FC<CreateClassModalProps> = ({
  ...
  const editConfiguration = editingClass && (
    <section className="space-y-5 rounded-xl border border-border/75 bg-card p-4 shadow-xs sm:p-5">
      {hasCycleChoice && (
      ...
      <div className="space-y-1.5">
        ...
        {customMode ? (
          ...
          <Select value={level} onValueChange={value => { setLevel(value); setLevelGroupKey(levelGroups.find(item => item.levels.includes(value))?.key ?? ''); }}>
            <SelectTrigger id="edit-class-level" className="!h-10 rounded-xl border-border bg-background text-sm"><SelectValue placeholder={copy.level} /></SelectTrigger>
      ...
      <div className="space-y-3">
        <div className="grid gap-1.5 sm:grid-cols-[7rem_minmax(0,1fr)] sm:items-center sm:gap-3">
          ...
          <Input id="edit-class-group" value={group} onChange={event => setGroup(sanitizeGroupNumberInput(event.target.value))} onBlur={() => { const value = normalizeGroupNumber(group); if (value) setGroup(value); }} inputMode="numeric" enterKeyHint="done" maxLength={2} aria-invalid={Boolean(groupError)} className="h-10 w-full rounded-xl border border-border bg-background text-center text-sm font-bold focus:border-primary" />
        </div>
        {showSubjectChoice && (
          <div className="grid gap-1.5 sm:grid-cols-[7rem_minmax(0,1fr)] sm:items-center sm:gap-3">
            ...
            {customMode ? (
              <Input id="edit-class-subject" value={customSubject} onChange={event => setCustomSubject(event.target.value)} placeholder={copy.customSubjectPlaceholder} className="h-10 rounded-xl border-border bg-background text-sm" />
            ) : (
              <Select value={subject} onValueChange={setSubject}><SelectTrigger id="edit-class-subject" className="!h-10 rounded-xl border-border bg-background text-sm"><SelectValue placeholder={copy.subjectPlaceholder} /></SelectTrigger><SelectContent className="rounded-xl">{subjectOptions.map(item => <SelectItem key={item} value={item}>{formatLocalizedSubjectDisplayName(item, locale)}</SelectItem>)}</SelectContent></Select>
            )}
          </div>
        )}
      </div>
  ...
  return (
    <>
      <Modal isOpen={isOpen} onClose={onClose} title={
        ...
        className="border border-border/80 bg-card shadow-xl sm:max-w-2xl sm:rounded-2xl"
        ...
        footer={<div className="flex w-full flex-col gap-2.5 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="grid grid-cols-2 gap-2 sm:ms-auto sm:flex sm:items-center">
            ...
            {step === 'cycle' && <Button type="button" onClick={() => chooseCycle(cycle)} className="h-10 w-full px-5 text-xs font-bold shadow-sm sm:w-auto sm:text-sm">{copy.next}{locale === 'ar' ? <ArrowLeft className="h-4 w-4" /> : <ArrowRight className="h-4 w-4" />}</Button>}
            {step === 'level' && customMode && <Button type="button" onClick={continueCustomLevel} disabled={!customLevel.trim()} className="h-10 w-full px-5 text-xs font-bold shadow-sm sm:w-auto sm:text-sm">{copy.next}{locale === 'ar' ? <ArrowLeft className="h-4 w-4" /> : <ArrowRight className="h-4 w-4" />}</Button>}
            {(editingClass || step === 'details') && <Button type="submit" form="class-form" disabled={!isFormValid} className="h-10 w-full px-5 text-xs font-bold shadow-sm sm:w-auto sm:text-sm">{editingClass ? copy.save : copy.create}</Button>}
          </div>
        </div>}
      >
        <form id="class-form" onSubmit={handleSubmit} dir={locale === 'ar' ? 'rtl' : 'ltr'} className="space-y-5 text-start">
          {!editingClass && <nav className="relative overflow-hidden rounded-xl border border-border bg-muted/40 px-3 py-4 shadow-xs sm:px-5 sm:py-5" aria-label={copy.guidedLabel}>
          ...
          {step === 'cycle' && hasCycleChoice && <section className="space-y-3"><label htmlFor="class-cycle" className="block text-sm font-bold text-foreground">{copy.cycle}</label><Select value={cycle} onValueChange={value => setCycle(value as Cycle)}><SelectTrigger id="class-cycle" className="!h-12 rounded-xl border-border bg-card px-4 text-sm"><SelectValue placeholder={copy.cyclePlaceholder} /></SelectTrigger><SelectContent className="rounded-xl">{cycleOptions.map(item => <SelectItem key={item} value={item}>{copy.cycleLabels[item]}</SelectItem>)}</SelectContent></Select></section>}
          ...
          {step === 'details' && <section className="space-y-3"><div className="rounded-xl border border-border/75 bg-card p-4 shadow-xs"><div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_7rem] sm:items-start"><div><p className="text-[10px] font-extrabold uppercase tracking-[0.14em] text-muted-foreground">{copy.selectedClass}</p><p className="mt-1 text-lg font-extrabold tracking-tight text-foreground">{selectedClassLabel}</p><p id="group-help" className={cn('mt-1 text-[11px]', groupError ? 'font-semibold text-destructive' : 'text-muted-foreground')}>{groupError ?? copy.groupHint}</p></div><div className="min-w-0 self-start"><label htmlFor="class-group" className="mb-1.5 block text-[10px] font-bold uppercase tracking-wider text-muted-foreground">{copy.group}</label><Input id="class-group" value={group} onChange={event => setGroup(sanitizeGroupNumberInput(event.target.value))} onBlur={() => { const value = normalizeGroupNumber(group); if (value) setGroup(value); }} placeholder="1–99" inputMode="numeric" enterKeyHint="done" maxLength={2} aria-describedby="group-help" aria-invalid={Boolean(groupError)} className="h-11 w-full rounded-xl border border-border bg-background text-center text-sm font-extrabold text-foreground shadow-none focus:border-primary" /></div></div>
