...
export const CalendarManager: React.FC<{ onBack: () => void }> = ({ onBack }) => {
    ...
    const save = async () => {
        ...
        try {
            ...
            setMessage('Calendrier publiÃ©. Les contrÃ´les de dates et le cron utilisent maintenant cette version.');
        } catch (error) {
    ...
    if (!calendar) return <div className="mx-auto max-w-5xl p-6"><button onClick={onBack}>â† Retour</button><p className="mt-6 text-sm text-muted-foreground">{message || 'Chargementâ€¦'}</p></div>;
        ...
        <main className="mx-auto max-w-6xl space-y-5 p-4 sm:p-8">
            <header className="flex flex-wrap items-center justify-between gap-3 rounded-2xl bg-foreground p-4 text-primary-foreground">
                <div className="flex items-center gap-3">
                    ...
                    <div><h1 className="text-lg font-black">Pilotage du calendrier</h1><p className="text-xs text-primary-foreground/65">Vacances, jours fÃ©riÃ©s et moteur de notifications</p></div>
                </div>
            ...
            <section className="rounded-2xl bg-secondary/60 p-4">
                <div className="grid gap-3 sm:grid-cols-3">
                    ...
                    <label className="text-xs font-bold">DÃ©but de l'annÃ©e<input type="date" value={calendar.anneeScolaire.debut} onChange={e => setCalendar({ ...calendar, anneeScolaire: { ...calendar.anneeScolaire, debut: e.target.value } })} className="mt-1 h-10 w-full rounded-xl border bg-card px-3 font-normal" /></label>
                    <label className="text-xs font-bold">Fin de l'annÃ©e<input type="date" value={calendar.anneeScolaire.fin} onChange={e => setCalendar({ ...calendar, anneeScolaire: { ...calendar.anneeScolaire, fin: e.target.value } })} className="mt-1 h-10 w-full rounded-xl border bg-card px-3 font-normal" /></label>
                </div>
            </section>

            <CalendarList
                title="Jours fÃ©riÃ©s"
            ...
            >
                {calendar.joursFeries.map((item, index) => (
                    <div key={`${item.date}-${index}`} className="grid gap-2 rounded-xl bg-card p-3 shadow-sm sm:grid-cols-[9rem_1fr_9rem_2.75rem]">
                        ...
                        <input value={item.nom} onChange={e => updateHoliday(index, { nom: e.target.value })} placeholder="Nom du jour fÃ©riÃ©" className="h-10 rounded-lg border px-3 text-xs" />
