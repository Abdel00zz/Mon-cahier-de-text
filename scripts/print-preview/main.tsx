...
import { findItem, deleteSeparator } from '../../utils/dataUtils';
...
const source: LessonsData = [{ type: 'chapter', title: ar ? 'الدوال العددية — الاستمرارية' : 'Fonctions numériques — Continuité', items: [
 ...
 { type: 'définition', title: ar ? 'الدوال المتصلة' : 'Fonctions continues', description: ar ? 'دراسة الاستمرارية باستعمال النهايات.' : 'Étude de la continuité à partir des limites.', date: '2026-09-22', separatorAfter: { content: ar ? 'نهاية الدرس $f(x)=x^2$' : 'Fin de séance $f(x)=x^2$', date: '2026-09-22', manual: true } },
...
function Preview() {
 ...
 return <LocaleProvider locale={ar?'ar':'fr'}><MathProvider>
  <div className="print-hidden mx-auto max-w-5xl p-5 space-y-4"><h1>Impression et séparateur — données de démonstration</h1>
   ...
   <MainTable lessonsData={data} visibleRows={buildLessonRows(data)} contentDirection={ar?'rtl':'ltr'} showDescriptions selectedKeys={new Set()} newlyAddedIds={[]} onClearSearch={()=>{}} onToggleSelect={()=>{}} onOpenContentEditor={()=>{}} onOpenAddContentModal={()=>{}}
     onCellUpdate={(indices,field,value)=>setData(d=>produce(d,draft=>{const {item}=findItem(draft,indices);if(item)(item as any)[field]=value;}))}
     onDeleteSeparator={indices=>setData(d=>produce(d,draft=>deleteSeparator(draft,indices)))}/>
  </div>
